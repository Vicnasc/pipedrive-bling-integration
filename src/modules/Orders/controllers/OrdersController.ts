...
import { SaveIntegrationMongoService } from '../services/SaveIntegrationMongo';
...
export class OrdersController {
  ...
  async handle(request: Request, response: Response) {
    try {
      ...
      const mongoResult = this.saveIntegration.execute(integrationDTO);

      return response.status(201).json(wonDeals);
    } catch (err) {
